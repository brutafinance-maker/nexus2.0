...
import { ASE10_DATA } from '../data/mock';

interface ASE10PageProps {
  onBack: () => void;
}

export const ASE10Page = ({ onBack }: ASE10PageProps) => {
      ...
      <Card className="overflow-hidden" hover={false}>
        <div className="p-8 bg-blue-600 dark:bg-blue-700 text-white">
          <div className="flex items-center gap-3 mb-2">
            <span className="px-3 py-1 bg-white/20 backdrop-blur-sm rounded-full text-xs font-bold uppercase tracking-wider">ASE 10</span>
          ...
          <h2 className="text-3xl font-bold">{ASE10_DATA.title}</h2>
          <p className="text-blue-100 dark:text-blue-200 mt-2">{ASE10_DATA.description}</p>
        </div>
        ...
        <div className="p-8">
          {subTab === 'problemas' && (
            <div className="space-y-4">
              {ASE10_DATA.problems.map((prob, i) => (
