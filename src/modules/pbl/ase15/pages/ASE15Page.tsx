...
import { ASE15_DATA } from '../data/mock';

interface ASE15PageProps {
  onBack: () => void;
}

export const ASE15Page = ({ onBack }: ASE15PageProps) => {
      ...
      <Card className="overflow-hidden" hover={false}>
        <div className="p-8 bg-emerald-600 dark:bg-emerald-700 text-white">
          <div className="flex items-center gap-3 mb-2">
            <span className="px-3 py-1 bg-white/20 backdrop-blur-sm rounded-full text-xs font-bold uppercase tracking-wider">ASE 15</span>
          ...
          <h2 className="text-3xl font-bold">{ASE15_DATA.title}</h2>
          <p className="text-emerald-100 dark:text-emerald-200 mt-2">{ASE15_DATA.description}</p>
        </div>
        ...
        <div className="p-8">
          {subTab === 'problemas' && (
            <div className="space-y-4">
              {ASE15_DATA.problems.map((prob, i) => (
